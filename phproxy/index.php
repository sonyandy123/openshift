...
$_labels            = array
                    (
                        'include_form'    => array('顯示工作列', '在每個頁面上方顯示工作列'), 
                        'remove_scripts'  => array('停用JavaScript', '停用JavaScript'), 
                        'accept_cookies'  => array('允許COOKIE', '允許儲存COOKIE'), 
                        'show_images'     => array('顯示圖片', '顯示圖片內容'), 
                        'show_referer'    => array('顯示真實網址', '顯示真實的提交網址'), 
                        'rotate13'        => array('Rotate13編碼', '網址使用ROT13編碼'), 
                        'base64_encode'   => array('Base64編碼', '網址使用base64編碼'), 
                        'strip_meta'      => array('省略Meta', '省略頁面的Meta標籤'), 
                        'strip_title'     => array('省略標題', '省略頁面標題'), 
                        'session_cookies' => array('會話COOKIE', '僅存儲此次會話的COOKIE') 
                    );
...
else
{
    ...
    {
        $_url_form      = '<div style="width:100%;margin:0;text-align:center;border-bottom:1px solid #725554;color:#222;background-color:#abcdef;font-size:12px;font-weight:bold;font-family:Bitstream Vera Sans,arial,sans-serif;padding:4px;">'
                        . '<form method="post" action="' . $_script_url . '">'
                        . ' <label for="____' . $_config['url_var_name'] . '"><a href="' . $_url . '">網址</a>:</label> <input id="____' . $_config['url_var_name'] . '" type="text" size="80" name="' . $_config['url_var_name'] . '" value="' . $_url . '" />'
                        . ' <input type="submit" name="go" value="送出" />'
                        . ' [前往: <a href="' . $_script_url . '?' . $_config['url_var_name'] . '=' . encode_url($_url_parts['prev_dir']) .' ">上層目錄</a>, <a href="' . $_script_base . '">首頁</a>]'
                        . '<br /><br />';
